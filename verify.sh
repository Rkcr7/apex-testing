...
echo "  CLI wall-clock = benchmark speed + file I/O + ~580ms CUDA startup."
echo "  The ~580ms CUDA startup happens ONCE when the process starts."
echo "  In real use (server, pipeline), CUDA loads once — all files after"
echo "  that run at benchmark speed. CPU-only binary has zero startup delay."
